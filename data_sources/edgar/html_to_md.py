...
"""HTML -> markdown-ish converter.

Ported from the old `profit` workspace so EDGAR narrative fields (notably
`*TextBlock*` elements) remain readable and roughly structured without adding
...
class Markdownifier(HTMLParser):
    ...
    def _close_marker(self, marker: str) -> None:
    ...
    def handle_endtag(self, tag: str) -> None:  # noqa: D401 - HTMLParser hook
        tag = tag.lower()
        if tag == "script":
            self.script_mode = False
            return
