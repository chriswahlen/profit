...
- Catalog is required: fetch scripts read lifecycle from the same SQLite file as the columnar store by default (override with `--store-path`) and will clip/skip requests outside `active_from/active_to`.
